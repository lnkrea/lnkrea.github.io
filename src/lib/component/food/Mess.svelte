<script lang="ts">
	...
	$: day_labels = [
		{ label: `yesterday (${days[today - 2]}'s menu)`, day: yest_day },
		{ label: `${heading}'s menu`, day: selected_day },
		{ label: `tomorrow (${days[today]}'s menu)`, day: next_day }
	];
...
<div class="week_day_selector">
	{#each days as day}
		<button class:selected={selected_day === day} on:click={() => selectDay(day)}>
